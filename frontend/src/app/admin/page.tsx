...
export default function AdminDashboardPage() {
  ...
  const [settings, setSettings] = useState<SystemSettings>({
    ...
    broadcastNotice:
      "Welcome to Pulse360! 24/7 Confidential Youth Health & Psychological Support in Rwanda.",
  });
  ...
  const [broadcasterTarget, setBroadcasterTarget] = useState<
    "all" | "Kigali" | "Huye" | "Rubavu" | "Musanze"
  >("all");
  const [broadcasterLang, setBroadcasterLang] = useState<"all" | "en" | "rw">(
    "all",
  );
  ...
  const [broadcasterSuccess, setBroadcasterSuccess] = useState<string | null>(
    null,
  );
  ...
  const fetchAdminStories = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        const storiesWithScores = (data.stories || []).map(
          (s: Record<string, unknown>) => ({
            ...s,
            aiSafetyScore:
              typeof s.aiSafetyScore === "number"
                ? s.aiSafetyScore
                : s.status === "approved"
                  ? 95
                  : s.status === "rejected"
  ...
  const handleUpdateStoryStatus = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        logAction(
          "Story Moderation Decision",
          `Set status of story ${id} to ${status}`,
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const handleSaveStoryContent = async () => {
    ...
    setStories((prev) =>
      prev.map((s) =>
        s.id === editingStoryId ? { ...s, content: storyEditContent } : s,
      ),
    );
  ...
  const handleBanSession = (sessionId: string) => {
    if (confirm(`Ban session ${sessionId} for abusive behavior?`)) {
      setSessions((prev) =>
        prev.filter((s) => s.sessionToken !== sessionId && s.id !== sessionId),
      );
  ...
  const handleDeleteSession = async (id: string) => {
    if (!confirm("Are you sure you want to terminate this anonymous session?"))
      return;
    try {
      ...
      if (data.success) {
        setClinicSuccess(
          editingClinic
            ...
            : "Clinic added to map directory!",
        );
        logAction(
          editingClinic ? "Clinic Updated" : "Clinic Added",
          `Managed clinic entry: ${clinicForm.name}`,
        );
  ...
  const handleDeleteClinic = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/clinics?id=${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleCreateArticle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setArticleSuccess(
          `Article successfully published as ${articleStatus}!`,
        );
        logAction(
          "Article Created",
          `Published: "${articleTitle}" (${articleLanguage})`,
        );
        setArticleTitle("");
        setArticleBody("");
  ...
  const handleCrisisAction = async (
    id: string,
    ...
    try {
      ...
      if (data.success) {
        ...
        logAction(
          "Crisis Action Triggered",
          `Crisis ${id} action taken: ${action}`,
        );
        alert(
          `Crisis alert ${action === "escalate" ? "escalated to emergency services" : "marked as resolved"}.`,
        );
      }
    } catch (e) {
      console.error(e);
    ...
    try {
      ...
      if (data.success) {
        ...
        logAction(
          ...
          "Retrained prompt instructions template.",
        );
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSettingsSuccess(
          "Platform settings, API integrations and model values updated.",
        );
        logAction(
          "Settings Changed",
          "Modified OpenAI, SMS, Maps, and theme parameters.",
        );
  ...
  const handleBroadcast = (e: React.FormEvent) => {
    ...
    setBroadcasterSuccess(
      `Successfully broadcasted ${broadcasterType} to ${broadcasterTarget} (${broadcasterLang})!`,
    );
    logAction(
      "Broadcast Dispatched",
      `Sent broadcast to ${broadcasterTarget} via ${broadcasterType}`,
    );
  ...
  const triggerExport = (format: "CSV" | "Excel" | "PDF") => {
    logAction(
      "Data Exported",
      `Downloaded system analytics report in ${format} format.`,
    );
    alert(
      `Success: Exported metrics and logs to Pulse360_Report.${format.toLowerCase()}`,
    );
  };
  ...
  const toggleUserRole = async (userId: string, currentRole: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        logAction(
          "User Role Changed",
          `Toggled role of user ${userId} to ${newRole}`,
        );
      }
    } catch (e) {
      console.error(e);
    ...
    try {
      ...
      if (data.success) {
        ...
        logAction(
          ...
          `Set status of user ${userId} to ${newStatus}`,
        );
      }
    } catch (e) {
      console.error(e);
    ...
    <div className="max-w-7xl mx-auto space-y-6 text-left animate-fade-in pb-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Secure administrative access console for health delivery telemetry
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 bg-white border border-[#edeaf5] p-4 rounded-3xl shadow-sm space-y-1">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider px-3 mb-2">
          ...
          <nav className="space-y-1">
            {[
              ...
              {
                id: "chat_monitoring",
                label: "3. AI Chat Monitoring",
                icon: Activity,
              },
              { id: "cms", label: "4. Articles (CMS)", icon: FileText },
              {
                id: "stories",
                label: "5. Community Stories",
                icon: MessageSquare,
              },
              { id: "clinics", label: "6. Clinic Locator", icon: MapPin },
              {
                id: "consultations",
                label: "7. Consultations",
                icon: UserCheck,
              },
              {
                id: "crisis_center",
                label: "8. Crisis Center",
                icon: AlertTriangle,
              },
              {
                id: "knowledge_base",
                label: "9. AI Knowledge Base",
                icon: BookOpen,
              },
              {
                id: "analytics",
                label: "10. Analytics Analytics",
                icon: TrendingUp,
              },
              {
                id: "payments",
                label: "11. Payments & MoMo",
                icon: CreditCard,
              },
              {
                id: "notifications",
                label: "12. Broadcaster Tool",
                icon: Megaphone,
              },
              ...
              if (!allowed) {
                return (
                  <div
                    key={tab.id}
                    className="flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold text-slate-300 bg-slate-50 cursor-not-allowed select-none"
                  >
              ...
              return (
                ...
                >
                  ...
                  {tab.id === "crisis_center" &&
                    crisisEvents.filter((c) => !c.resolved).length > 0 && (
                ...
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      },
                      {
                        ...
                        value: stories.filter((s) => s.status === "pending")
                          .length,
                        icon: Clock,
                      ...
                      },
                      {
                        ...
                        value: crisisEvents.filter((c) => !c.resolved).length,
                        icon: AlertTriangle,
                        color: "text-rose-600",
                        bg: "bg-rose-50",
                      },
                      {
                        label: "Revenue (MTN / Airtel)",
                        value: "185,000 RWF",
                        icon: CreditCard,
                      ...
                      return (
                        <div
                          key={idx}
                          className="p-4 rounded-3xl bg-white border border-[#edeaf5] shadow-sm flex items-center gap-3 hover:scale-[1.01] transition-transform"
                        >
                          ...
                          <div>
                            ...
                            <h4 className="text-base font-black text-[#2d1c66] mt-1">
                              {kpi.value}
                            </h4>
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                      <h4 className="text-xs font-black text-[#2d1c66] uppercase tracking-wider">
                        AI Support Inquiry Topics
                      </h4>
                      <div className="space-y-3 pt-2">
                        {[
                          ...
                          {
                            topic: "General Wellbeing, Fitness & Nutrition",
                            percentage: 13,
                    ...
                    <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                      ...
                      <div className="space-y-2 pt-1.5">
                        {[
                          ...
                          {
                            query:
                              "Where to find anonymous SRH testing near Kigali?",
                            searchCount: 54,
                          },
                          {
                            query:
                              "Confidential psychological counselors hotline.",
                            searchCount: 42,
                          },
                          {
                            query: "Common myths about reproductive health.",
                            searchCount: 31,
                          },
                        ].map((item, i) => (
                          <div
                            key={i}
                            className="flex items-center justify-between p-2.5 rounded-xl bg-[#f7f6fc] border border-[#edeaf5] text-xs"
                          >
                            <span className="font-bold text-[#2d1c66] truncate max-w-[200px]">
                              {item.query}
                            </span>
                  ...
                  <tbody className="divide-y divide-[#edeaf5] text-xs text-[#2d1c66]">
                    {loadingSessions ? (
                      ...
                      sessions.map((sess) => (
                        <tr
                          key={sess.id}
                          className="hover:bg-[#f7f6fc]/50 transition"
                        >
                          <td className="p-4 font-mono font-extrabold text-purple-700">
                            @{sess.id.toUpperCase()}
                          </td>
                          <td className="p-4 font-semibold">
                            {sess.district}, {sess.country}
                          </td>
                          <td className="p-4">
                            <span className="uppercase bg-slate-100 px-2 py-0.5 rounded text-[10px] font-bold">
                              {sess.language}
                            </span>
                          </td>
          ...
          {activeTab === "chat_monitoring" && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-white border border-[#edeaf5] rounded-2xl shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      logAction(
                        "AI Dataset Retrain Triggered",
                        "Manually queued retrain sequence on active logs.",
                      );
                      alert(
                        "AI Model dataset retraining queued. Processes will complete asynchronously.",
                      );
                    }}
              ...
              <div className="space-y-4">
                {loadingChats ? (
                  <div className="p-8 text-center bg-white border border-[#edeaf5] rounded-2xl text-slate-400 text-xs">
                  ...
                  filteredChats.map((chat) => (
                    <div
                      key={chat.id}
                      className="p-5 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4"
                    >
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase border-b border-[#edeaf5] pb-2">
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-0.5 rounded bg-purple-50 text-purple-700 font-extrabold">
                            {chat.category}
                          </span>
                          <span className="text-slate-400 font-mono">
                            Session: @{chat.sessionId.toUpperCase()}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-emerald-600">
                            AI Confidence: {Math.round(chat.confidence * 100)}%
                          </span>
                          <span className="text-amber-500">
                            Rating: {chat.feedbackRating}/5 ⭐
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3 text-xs leading-relaxed">
                        <div className="flex items-start gap-2 bg-[#f7f6fc] p-3 rounded-2xl">
                          <span className="font-extrabold text-[#7c3aed] uppercase shrink-0">
                            User:
                          </span>
                          <p className="text-[#2d1c66]">
                        ...
                        <div className="flex items-start gap-2 bg-purple-50/50 p-3 rounded-2xl border border-purple-100">
                          <span className="font-extrabold text-[#ec4899] uppercase shrink-0">
                            AI:
                          </span>
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => {
                              logAction(
                                "AI Response Flagged",
                                `Flagged incorrect response in session ${chat.sessionId}`,
                              );
                              alert(
                                "This response has been flagged for clinic-based expert review.",
                              );
                            }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setArticleStatus(
                            e.target.value as typeof articleStatus,
                          )
                        }
              ...
              <div className="lg:col-span-5 p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                ...
                <div className="space-y-3">
                  {[
                    {
                      title: "Understanding Anxiety for Young Rwandans",
                      lang: "English",
                      cat: "Mental Health",
                      status: "Published",
                    },
                    {
                      title: "Gusobanukirwa Agahinda Gakabije",
                      lang: "Kinyarwanda",
                      cat: "Mental Health",
                      status: "Published",
                    },
                    ...
                    <div
                      key={i}
                      className="p-3.5 rounded-2xl bg-[#f7f6fc] border border-[#edeaf5] flex flex-col gap-2"
                    >
                      <div className="flex justify-between items-center text-[10px] font-black uppercase text-indigo-600">
                        <span>{art.cat}</span>
                        <span
                          className={`px-2 py-0.5 rounded ${art.status === "Published" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                        >
                          {art.status}
                        </span>
                      </div>
                      <h4 className="text-xs font-bold text-[#2d1c66]">
                        {art.title}
                      </h4>
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg text-[10px] font-extrabold uppercase transition ${
                          storyFilter === f
                            ...
                            : "text-slate-400 hover:text-[#2d1c66]"
                        }`}
                      >
                        {f}
                      </button>
              ...
              <div className="space-y-3">
                {loadingStories ? (
                  ...
                  filteredStories.map((story) => (
                    <div
                      key={story.id}
                      className="p-5 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-3"
                    >
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-400">
                            Location: {story.districtHash}
                          </span>
                        </div>

                        <div className="flex items-center gap-2">
                          <span
                            className={`px-2 py-0.5 rounded font-bold ${story.aiSafetyScore > 60 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                          >
                            AI Moderation Safety: {story.aiSafetyScore}%
                          </span>
                          <span
                            className={`px-2.5 py-0.5 rounded-md border font-extrabold ${
                      ...
                      {editingStoryId === story.id ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            onChange={(e) =>
                              setStoryEditContent(e.target.value)
                            }
                      ...
                      <div className="flex items-center justify-between pt-2 text-[10px] text-slate-400">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() =>
                              handleUpdateStoryStatus(story.id, "rejected")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleUpdateStoryStatus(story.id, "approved")
                            }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in">
                ...
                <div className="space-y-3">
                  {loadingClinics ? (
                    ...
                    filteredClinics.map((clinic) => (
                      <div
                        key={clinic.id}
                        className="p-4 rounded-2xl bg-white border border-[#edeaf5] shadow-sm space-y-3"
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-extrabold text-xs text-[#2d1c66]">
                              {clinic.name}
                            </h4>
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-bold ${clinic.status === "Open" ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                          >
                            {clinic.status}
                          </span>
                        </div>
              ...
              <div className="lg:col-span-6 space-y-4">
                <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                  <h3 className="font-extrabold text-sm text-[#2d1c66] uppercase tracking-wider">
                    {editingClinic
                      ? "Edit Referral Center"
                      : "Add Clinic via Map Locator"}
                  </h3>
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase">
                      Kigali Interactive Map Simulator (Click to pick lat/lng)
                    </label>
                    <div
                    ...
                    >
                      ...
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                        <span className="text-[10px] font-extrabold text-[#2d1c66] bg-white/80 px-2 py-0.5 rounded-full border">
                  ...
                  <form onSubmit={handleSaveClinic} className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input
                          type="text"
                          readOnly
                          value={clinicForm.lat}
                          className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs font-mono"
                        />
                      </div>
                      <div>
                        <label className="text-[9px] font-bold text-slate-400 uppercase">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs font-bold"
                        >
                    ...
                    <button
                      type="submit"
                      className="w-full py-2 bg-[#7c3aed] text-white text-xs font-extrabold rounded-xl hover:bg-purple-700 transition"
                    >
          ...
          {activeTab === "consultations" && (
            <div className="space-y-6 animate-fade-in">
              <div className="p-4 bg-white border rounded-2xl shadow-sm flex items-center justify-between">
                <h3 className="font-extrabold text-sm text-[#2d1c66] uppercase tracking-wider">
                  Live Telehealth Consultations & Meeting Rooms
                </h3>
                    ...
                    {loadingConsultations ? (
                      ...
                      <tr>
                        <td
                          ...
                          className="p-8 text-center text-slate-400 text-xs"
                        >
                          No consultation bookings found yet.
                        </td>
                      </tr>
                    ) : (
                      consultations.map((apt) => (
                        <tr
                          key={apt.id}
                          className="hover:bg-[#f7f6fc]/50 transition align-top"
                        >
                          <td className="p-4">
                            <div className="font-bold">{apt.counselorName}</div>
                            <div className="uppercase text-[10px] font-black text-slate-400 mt-1">
                              {apt.counselorRole}
                            </div>
                          </td>
                          <td className="p-4">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {(
                                ...
                                <button
                                  key={status}
                                  type="button"
                                  disabled={updatingConsultationId === apt.id}
                                  onClick={() =>
                                  ...
                                  className={`px-2 py-1 rounded text-[9px] font-bold border transition ${
                                    apt.status === status
                                      ...
                                      : "bg-white text-slate-400 border-[#edeaf5] hover:bg-[#f7f6fc]"
                                  }`}
                                >
                                  {status}
                                </button>
                          ...
                          <td className="p-4 text-right space-y-2">
                            <a
                              ...
                              className="inline-block px-3 py-1 rounded-xl bg-purple-50 text-[#7c3aed] border border-purple-100 hover:bg-purple-100 transition text-[10px] font-bold"
                            >
                              Open WebRTC Room
                            </a>
              ...
              <div className="p-5 bg-red-50 border border-red-200 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-red-700">
                    If AI triggers self-harm risks, dispatch protocols instantly
                    to local emergency hotlines or text responders.
                  </p>
                </div>
                <div className="flex gap-2">
                  <a
              ...
              <div className="space-y-4">
                {loadingCrisis ? (
                  ...
                  crisisEvents.map((event) => (
                    <div
                      key={event.id}
                      className="p-5 rounded-3xl bg-white border border-[#edeaf5] shadow-sm flex flex-col md:flex-row md:items-start justify-between gap-6"
                    >
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs text-[#2d1c66] font-bold">
                          Triggered in anonymous chat session:{" "}
                          <span className="font-mono text-purple-700">
                            @{event.sessionId.substring(0, 8)}
                          </span>
                        </p>
                        <p className="text-[10px] text-slate-400">
                          Report Location: {event.district} | Trigger Timestamp:{" "}
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 md:justify-end">
                        <button
                          onClick={() => {
                            logAction(
                              ...
                              `Assigned counselor to crisis event ${event.id}`,
                            );
                            alert(
                              "Counselor has been alerted and connected to the session anonymously.",
                            );
                          }}
                        ...
                        <button
                          onClick={() => {
                            logAction(
                              "Crisis emergency SMS sent",
                              `Dispatched location text coordinates for event ${event.id}`,
                            );
                            alert(
                              "Confidential crisis response SMS dispatched to district healthcare nurse.",
                            );
                          }}
                        ...
                        <button
                          onClick={() =>
                            handleCrisisAction(
                              event.id,
                              event.resolved ? "escalate" : "resolve",
                            )
                          }
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                ...
                <form onSubmit={handleUpdateSystemPrompt} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="px-5 py-2.5 rounded-xl bg-purple-600 text-white text-xs font-bold hover:bg-purple-700 transition shadow-sm"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 bg-white border rounded-3xl shadow-sm space-y-4">
                  <h4 className="text-xs font-bold text-[#2d1c66] uppercase">
                    Reference Guidelines (RAG Resources)
                  </h4>
                  <div className="space-y-2">
                    {knowledgeBase?.guidelines.map((g, idx) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-3 rounded-2xl bg-[#f7f6fc] border text-xs"
                      >
                        <span className="font-bold text-[#2d1c66]">
                          {g.title} ({g.source})
                        </span>
                ...
                <div className="p-6 bg-white border rounded-3xl shadow-sm space-y-4">
                  <h4 className="text-xs font-bold text-[#2d1c66] uppercase">
                    Active Trained FAQs ({knowledgeBase?.faqs.length || 0})
                  </h4>
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                    {knowledgeBase?.faqs.map((faq) => (
                      <div
                        key={faq.id}
                        className="p-3.5 rounded-2xl bg-[#f7f6fc] border border-[#edeaf5] text-xs space-y-1"
                      >
                        <h5 className="font-bold text-[#2d1c66]">
                          Q: {faq.question}
                        </h5>
            ...
            <div className="space-y-6 animate-fade-in">
              <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Generate and download platform telemetry reports
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => triggerExport("CSV")}
                      className="px-3 py-1.5 rounded-xl bg-purple-50 text-[#7c3aed] text-xs font-bold hover:bg-purple-100 transition"
                    >
                      CSV
                    </button>
                    <button
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  ...
                  <div className="p-4 bg-[#f7f6fc] border rounded-2xl space-y-3">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-wider">
                      Consultation Referral Success Rate
                    </h4>
                    <div className="flex items-center gap-3">
                            ...
                            {pay.status === "completed" && (
                              <button
                                onClick={() => {
                                  if (
                                    confirm(
                                      `Issue refund for ${pay.paymentRef}?`,
                                    )
                                  ) {
                                    setPayments((prev) =>
                                      prev.map((p) =>
                                    ...
                                    logAction(
                                      "Payment Refunded",
                                      `Refunded transaction ref: ${pay.paymentRef}`,
                                    );
                                  }
                                }}
            ...
            <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
              <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-4">
                ...
                <form onSubmit={handleBroadcast} className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setBroadcasterTarget(
                            e.target.value as typeof broadcasterTarget,
                          )
                        }
                        className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs"
                      >
                        <option value="all">
                          All Districts (Entire Rwanda)
                        </option>
                    ...
                    <div>
                      ...
                      <select
                        value={broadcasterLang}
                        onChange={(e) =>
                          setBroadcasterLang(
                            e.target.value as typeof broadcasterLang,
                          )
                        }
                        className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setBroadcasterType(
                            e.target.value as typeof broadcasterType,
                          )
                        }
                        className="w-full p-2 bg-[#f7f6fc] border rounded-xl text-xs font-bold"
                      >
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-indigo-600 text-white text-xs font-extrabold rounded-xl hover:bg-purple-700 transition"
                  >
            ...
            <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
              <div className="p-6 rounded-3xl bg-white border border-[#edeaf5] shadow-sm space-y-5">
                ...
                <form onSubmit={handleSaveSettings} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full p-2.5 bg-[#f7f6fc] border rounded-xl text-xs"
                      >
                        <option value="gpt-4o">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSettings({ ...settings, mapsKey: e.target.value })
                        }
                        className="w-full p-2.5 bg-[#f7f6fc] border rounded-xl text-xs"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-2.5 bg-[#f7f6fc] border rounded-xl text-xs font-mono"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 bg-[#7c3aed] text-white text-xs font-bold rounded-xl hover:bg-purple-700 transition"
                  >
          ...
          {activeTab === "roles" && (
            <div className="space-y-6 animate-fade-in">
                  ...
                  <tbody className="divide-y divide-[#edeaf5]">
                    {[
                      ...
                      >
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] ${row.analytics === "Allowed" ? "bg-emerald-50 text-emerald-600 font-bold" : "text-slate-400"}`}
                          >
                            {row.analytics}
                          </span>
                        </td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] ${row.cms === "Allowed" ? "bg-emerald-50 text-emerald-600 font-bold" : "text-slate-400"}`}
                          >
                            {row.cms}
                          </span>
                        </td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] ${row.mod === "Allowed" ? "bg-emerald-50 text-emerald-600 font-bold" : "text-slate-400"}`}
                          >
                            {row.mod}
                          </span>
                        </td>
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] ${row.crisis === "Allowed" ? "bg-emerald-50 text-emerald-600 font-bold" : "text-slate-400"}`}
                          >
                            {row.crisis}
                          </span>
                        </td>
                        <td className="p-4 text-right">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] ${row.settings === "Allowed" ? "bg-emerald-50 text-emerald-600 font-bold" : "text-slate-400"}`}
                          >
                            {row.settings}
                          </span>
                        </td>
              ...
              <div className="bg-white p-6 border rounded-3xl shadow-sm space-y-4">
                <h4 className="text-xs font-black uppercase text-[#2d1c66]">
                ...
                <div className="space-y-2">
                  {loadingUsers ? (
                    ...
                    users.map((u) => (
                      <div
                        key={u.id}
                        className="p-3 bg-[#f7f6fc] border rounded-2xl flex items-center justify-between text-xs"
                      >
